...
void Fakerate::EventLoop(const char *data,const char *inputFileList) {
  ...
  int lep=0,nele=0,npho=0,npho_px=0,nele_px=0,remain=0,tag_ele=0,probe_ele=0,elec_reco=0,before_eletrig=0,after_eletrig=0,met=0,st=0,iso=0,mt=0,one_e2=0,two_e2=0,one_e=0,two_e=0;
    ...
    if(nlep==1)
      {
        ...
        double mindr2=100;
     	...
     	if(!s_data.Contains("data")) { mindr=tmp.DeltaR(v_genz[0]); mindr2=-1; mindr2=bestPhoton.DeltaR(v_genz[1]);}
	if(s_data.Contains("data")) 
	  {
