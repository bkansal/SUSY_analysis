...
void plotratio_LE_newselec(TString varname,TString title1,bool ExpvsPred, bool SRvsCR)
{
  ...
  if(varname=="AllSBins_v7_CD_SP" || varname=="AllSBins_v6_CD_EW_50bin" || varname=="AllSBins_v6_CD")
    {
  ...
  for(int i=0; i<k;i++)
	...
	h_3_sr->Rebin(rebin);
      ...
      if(varName[i]=="nJets" || varName[i]=="nBTags" || varName[i]=="Qmulti"){
	name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	...
	if(varName[i]=="METvBin2"){
      ...
      if(varName[i]=="hadAk8jetPt" || varName[i]=="MET" ||  varName[i]=="ST" || varName[i]=="EMObj_Pt" ){
        name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
      ...
      else if(SRvsCR)
	{
	  if(lep=="LE")
	    ...
	    {
	      TF[i]->GetYaxis()->SetRangeUser(0,5.0);
