...
void Fakerate::EventLoop(const char *data,const char *inputFileList) {
  ...
  int lep=0,nele=0,npho=0,npho_px=0,nele_px=0,remain=0,tag_ele=0,probe_ele=0,elec_reco=0,before_eletrig=0,after_eletrig=0,met=0,st=0,iso=0,mt=0,one_e2=0,two_e2=0,one_e=0,two_e=0,two_e3=0;
  //  cout<<"-=====> jentry : Mindr(tmp,gene1) : Mindr(tmp2,gene2)  :  Mindr(tmp,gene2) : Mindr(tmp2,gene1)  : "<<endl;
  ...
  // for (Long64_t jentry=0; jentry<300000;jentry++) {
   ...
   if(!s_data.Contains("data"))
       ...
       if(hasGenPromptPhoton && (s_data.Contains("ZGJets")))
	 {
       ...
       if(hasGenPromptPhoton && (s_data.Contains("DYJets2")))
         {
           if(!(madMinPhotonDeltaR<=0.5 || mindr_Pho_genlep<=0.5) ) continue;
	 }
       //Gen non-prompt
   ...
   // //apply iso track veto                                                                                                                
      ...
      if(!bestPhoHasPxlSeed && bestPhoton.Pt()>100)
	{
	  ...
	  // if((*Photons_passElectronVeto)[bestPhotonIndxAmongPhotons]!=1)cout<<jentry<<" : "<<(*Photons_passElectronVeto)[bestPhotonIndxAmongPhotons]<<endl; 
	}//npho++;}
	...
	for(int i=0 ; i<Electrons->size() ; i++)
	  {
		...
		if(nlep==1)
		  { 
		    e_index=i;
		    tmp=(*Electrons)[e_index];
		    one_e++;
		  }
		if(nlep==2) 
		  {
		    e_index2=i;
		    tmp2=(*Electrons)[e_index2];
		    two_e++;
		    //	    if((*Electrons)[e_index2].Pt()<100) cout<<jentry<<" : "<<(*Electrons)[e_index].Pt()<<" , "<<(*Electrons)[e_index2].Pt()<<endl;
		  }
	      }
	  }
      }
    ...
    //    cout<<jentry<<" : tag pt = "<<tagEMObj.Pt()<<" , Probe Pt = "<<bestEMObj.Pt()<<endl;
