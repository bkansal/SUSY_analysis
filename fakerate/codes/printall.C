void printall(bool lowdphi=false,bool syst=false, TString year="full_Run2",int nbin=52, int xmin=0, int xmax=52)
{
  ...
  if(!lowdphi && syst)  fout=new TFile("Fakerate_highdphi_sys.root","recreate");
  fout->cd();
  ...
  if(print)
      ...
      for(int i=1;i<n;i++)
	{
	  ...
	  if(!lowdphi && syst)
	    h_preddata->SetBinError(i+1,pred_sr->GetBinContent(i+1)*pow((pow(h_fr->GetBinError(i+1)/h_fr->GetBinContent(i+1),2) + pow(cr->GetBinError(i+1)/cr->GetBinContent(i+1),2) + pow(syserr->GetBinError(i),2)),0.5));
	  else  h_preddata->SetBinError(i+1,pred_sr->GetBinContent(i+1)*pow((pow(h_fr->GetBinError(i+1)/h_fr->GetBinContent(i+1),2) + pow(cr->GetBinError(i+1)/cr->GetBinContent(i+1),2)),0.5));
	}
    }
