void printall(bool lowdphi=false, TString year="full_Run2",int nbin=52, int xmin=0, int xmax=52)
{
  ...
  // rootout/fake_rate_v10_afterbtagSF/FR_using_PhoPtQmulti_csv_cut_EMObjeta_qmulti_METCaloMET_nopxveto_SF_bSF_lowdphi_newbin2/
  if(lowdphi) {
    ...
    path_bsf="rootout/fake_rate_v10_afterbtagSF/FR_using_PhoPtQmulti_csv_cut_EMObjeta_qmulti_METCaloMET_nopxveto_noSF_bSFuperror_newbin2/";
  ...
  if(!lowdphi) {
    ...
    path2="rootout/fake_rate_v10_afterbtagSF/FR_using_PhoPtQmulti_csv_cut_EMObjeta_qmulti_METCaloMET_nopxveto_SF_bSF_newbin2/Run2_METdata_CR_v18.root";
  ...
  int n=10;
  bool print=true;
  TFile *f_syserr= new TFile("FR_sys.root");

  //  if(!lowdphi && year=="full_Run2")    TF = TFile::Open("factors/fake_rate_highdphi.root","READ");
  //  if(lowdphi && year=="full_Run2")    TF = TFile::Open("factors/fake_rate_lowdphi.root","READ");
  if(!lowdphi && year=="full_Run2")    TF = TFile::Open("fake_rate_highdphi.root","READ");
  if(lowdphi && year=="full_Run2")    TF = TFile::Open("fake_rate_lowdphi.root","READ");
  ...
  filename2_  = path1+"TTWGJ_CR_v18.root";
  ...
  TFile *fa = new TFile("rootout/new_noSF_v2/Run2016_METdata_CR_v18.root");
  TFile *fb = new TFile("rootout/new_noSF_v2/Run2017_METdata_CR_v18.root");
  TFile *fc = new TFile("rootout/new_noSF_v2/Run2018_METdata_CR_v18.root");

  TH1D *cr_a     = (TH1D*)fa->Get("AllSBins_v6_CD_EW_50bin_elec1");
  TH1D *cr_b     = (TH1D*)fb->Get("AllSBins_v6_CD_EW_50bin_elec1");
  TH1D *cr_c     = (TH1D*)fc->Get("AllSBins_v6_CD_EW_50bin_elec1");

  for(int q=1; q<=46; q++)
    {
      sf_mjb[q]=((1.6244*cr_a->GetBinContent(q+1)) + (2.8378*cr_b->GetBinContent(q+1)) + (1.7967*cr_c->GetBinContent(q+1)))/cr->GetBinContent(q+1);
      sferr_mjb[q]=pow(((0.0621575*0.0621575*cr_a->GetBinContent(q+1)) + (0.0893453*0.0893453*cr_b->GetBinContent(q+1)) + (0.0501307*0.0501307*cr_c->GetBinContent(q+1)))/cr->GetBinContent(q+1),0.5);
  ...
  if(!lowdphi) fout=new TFile("Fakerate_highdphi.root","recreate");
  if(lowdphi)  fout=new TFile("Fakerate_lowdphi.root","recreate");
  fout->cd();
  ...
  if(print)
      ...
      for(int i=1;i<n;i++)
	{
	  ...
	  //	  cout<<sf_mjb[i]*pow((pow(cr->GetBinContent(i+1)*frerr_mjb[i],2) + pow(cr->GetBinError(i+1)*fr_mjb[i],2) + pow(syserr->GetBinError(i),2)),0.5)<<endl;
	  ...
	  if(!lowdphi) h_preddata->SetBinError(i+1,pred_sr->GetBinContent(i+1)*pow((pow(frerr_mjb[i]/fr_mjb[i],2) + pow(cr->GetBinError(i+1)/cr->GetBinContent(i+1),2) + pow(syserr->GetBinError(i)/pred_sr->GetBinContent(i+1),2)),0.5));
	}
    }
