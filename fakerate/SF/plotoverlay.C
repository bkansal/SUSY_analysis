void plotoverlay(TString varName,  TString year="full_Run2",  TString ntuples, int rebin, bool data=false)
{
    gStyle->SetOptStat(kFALSE);

    TFile *f1, *f2,*f, *f3,*f4,*f6;
  TString path, path1,path2;
  path1="./rootoutput/v1/withsimpleselections_ST200_nopxveto/";
  ...
  double ymin=0,ymax=1.99,xmin=0,xmax=0, ymin_=0.5, ymax_=10000000;
  //  bool data=false;
  ...
  if(!data){
   ...
   if(data){
    ...
    //    filename1= path1+"TTWDYG_2018_CR_v18.root";
  ...
  if(ntuples=="v3") 
      ...
      if(!data)
	{
      ...
      if(varName=="cutflows")
	h_DY_cr->GetXaxis()->SetNdivisions(-1);
