...
void plotratio_LE_newselec(TString varname,TString title1,bool ExpvsPred, bool SRvsCR)
{
  ...
  //  TString path = "/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v2/FR_for2016_px/";
  //  TString path = "/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v2/FR_for2016_wopx/";
  //  TString path = "./rootout/fake_rate_v4/FR_using_PhoPtQmulti/";
  //  TString path = "./rootout/fake_rate_v4/FR_using_PhoPtnJets/";
  //  TString path = "./rootout/fake_rate_v1/FR_v1/FR_foryearwise/";
  //  TString path="/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v3/FR_using_PhoPtnJets/";
  TString path ;
 path= "/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v5/FR_using_PhoPtnJetsQmulti_csv_cut/";
 ...
 //  path="/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v6/FR_using_PhoPtQmulti_csv_cut_usingtrg_reject_notmatchede_HEM_pucenter/";
  //  path="/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v6/FR_using_PhoPtQmulti_csv_cut_usingtrg_reject_notmatchede_HEM/";
      path="/Users/bkansal/work/MET_Analysis/Analysis/fakerate/rootout/fake_rate_v7/FR_using_PhoPtQmulti_csv_cut_EMObjeta_qmulti_METCaloMET_lowdphi/";
 ...
 TString path1;
  ...
  if(title1=="full_Run2"){
  ...
  else
    {
      ...
      /* f2 = new TFile(path1); */
      /* path1=path+"TTWDYJ_"+title1+"_CR_v18.root"; */
      /* //path1=path+"TTGJets_"+title1+"_CR_v18_v2.root"; */
      /* f1 = new TFile(path1); */
  ...
  if(varname=="AllSBins_v7_CD_SP" || varname=="AllSBins_v6_CD_EW_50bin" || varname=="AllSBins_v6_CD" || varname=="AllSBins_v6_CD_SP")
    {
  ...
  for(int i=0; i<k;i++)
	...
	TF[i]= new TH1D("tf_SP_4","Transfer factor for SP : NJets x Nbtag & EW : wtag & htag",nbin,bin0,bin1);
      ...
      else if(varName[i]=="AllSBins_v6_CD"){
	name1="Bin no.";
	cout<<"cr : "<<varName2[i]<<endl;
	cout<<"sr : "<<varName1[i]<<endl;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	h_1_cr=(TH1D*)f_1->FindObjectAny(varName2[i]);
	...
	yset_=200;
	/* nbin=44, bin0=0, bin1=50; */
	/* xmin_=1,xmax_=5; */
	nbin=48, bin0=1, bin1=49;
	xmin_=0,xmax_=44;
	total = new TH1D("total","Total = fail_id+fail_iso+fail_accept+1e_cr",nbin,bin0,bin1);
	TF[i]= new TH1D("tf_SP_4","Transfer factor for SP : NJets x Nbtag & EW : wtag & htag",nbin,bin0,bin1);
      }
	...
	h_2_sr=(TH1D*)f_2->FindObjectAny(varName1[i]);
	h_3_sr=(TH1D*)f_3->FindObjectAny(varName1[i]);
	if(lep=="LE"||lep=="LM" )	ymin_=0.1, ymax_=1000;
	if(lep=="LL" )	ymin_=1, ymax_=10000;
	//	ymin_=1, ymax_=1000;
	...
	h_3_sr->Rebin(rebin);
      ...
      if(varName[i]=="nJets" || varName[i]=="nBTags" || varName[i]=="Qmulti" || varName[i]=="NEMobj"){
	int rebin=1;
	name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	...
	else if(varName[i]=="nBTags" || varName[i]=="NEMobj")
	...
	if(lep=="LE"||lep=="LM")	ymin_=0.1, ymax_=100000;
	if(lep=="LL" )	ymin_=0.1, ymax_=100000;
	//	ymin_=1, ymax_=1000;
	...
	if(varName[i]=="METvBin2"){
      ...
      if(varName[i]=="h_minDr_bestphoJets"){
        name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	...
	int rebin;
	rebin=2;
	nbin=500/rebin, bin0=0, bin1=5;
	xmin_=0,xmax_=0.6;
	total = new TH1D("total","Total = fail_id+fail_iso+fail_accept+1e_cr",nbin,bin0,bin1);
	TF[i]= new TH1D("tf_SP_2","Transfer factor for SP : NJets x Nbtag & EW : wtag & htag",nbin,bin0,bin1);
      }
      if(varName[i]=="BestPhotonEta" || varName[i]=="BestPhotonPhi"|| varName[i]=="JetEta1" || varName[i]=="JetEta2" || varName[i]=="JetEta3"|| varName[i]=="JetEta4" || varName[i]=="JetPhi1" || varName[i]=="JetPhi2" || varName[i]=="JetPhi3"|| varName[i]=="JetPhi4"){
        name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	...
	h_3_sr->Rebin(rebin);
      ...
      if( varName[i]=="dPhi_MET_CaloMET" || varName[i]=="dPhi_METjet1" || varName[i]=="dPhi_METjet2" || varName[i]=="dPhi_METjet3" || varName[i]=="dPhi_METjet4" || varName[i]=="dPhi_phoMET"){
        name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	...
	h_3_sr->Rebin(rebin);
      ...
      if(varName[i]=="hadAk8jetPt" || varName[i]=="MET" ||  varName[i]=="ST" || varName[i]=="EMObj_Pt" || varName[i]=="JetPt1" || varName[i]=="JetPt2" || varName[i]=="JetPt3"|| varName[i]=="JetPt4"){
        name1=varname;
	cr          = (TH1D*)f1->Get(varName2[i]);
	sr          = (TH1D*)f1->Get(varName1[i]);
	h_0_cr=(TH1D*)f_0->FindObjectAny(varName2[i]);
	...
	else if(varName[i]=="MET" || varName[i]=="ST")
	  {
	    rebin=5;
	...
	TArrow *arrow1 = new TArrow( -0.5,yset_, 8,yset_,0.01,"<|>");
	TArrow *arrow2 = new TArrow( 8,yset_, 14,yset_,0.01,"<|>");
	TArrow *arrow3 = new TArrow(14,yset_, 20,yset_,0.01,"<|>");
	TArrow *arrow4 = new TArrow(10,yset_, 26,yset_,0.01,"<|>");
	...
	TArrow *arrow8 = new TArrow(45,yset_, 52,yset_,0.01,"<|>");
	...
	Tl.DrawLatex(44.0,2*yset_,"EW : H tag");
      ...
      if(varName[i]=="AllSBins_v6_CD" || varName[i]=="AllSBins_v6_CD_SP"){
	...
	Tl.DrawLatex(40.2,10,">=500");
      ...
      else if(SRvsCR) legend->SetHeader("SR : 0 e & 1 photon","C"); 
	...
	Tl.DrawLatex(40.2,10,">=500");
      ...
      if(SRvsCR)  legend1->SetHeader("CR : 1 e & 0 photon","C"); 
      ...
      else if(SRvsCR)
	{
      ...
      /* cout<<"The SR content in bin"<<endl; */
      for(int j=bin0;j<=bin1;j++)
      	{ cout<<j<<" : "<<TF[i]->GetBinLowEdge(j)<<" : "<<TF[i]->GetBinContent(j)<<endl;}
      /* cout<<"==============="<<endl; */
      if(varName[i]=="METvBin2")
	for(int j=bin0;j<=METLowEdge_v2.size()-1;j++)
      ...
      if(varName[i]=="AllSBins_v6_CD_EW_50bin"  ){
      	...
      	TF[i]->GetXaxis()->SetTitle("Bin no.");
      	TF[i]->GetXaxis()->SetTitleSize(0.17);
      	TLine *line1V6=new TLine( 8,0,  6.5,ymax_);
      	TLine *line2V6=new TLine(14,0, 12.5,ymax_);
      	TLine *line3V6=new TLine(20,0, 18.5,ymax_);
      	TLine *line4V6=new TLine(26,0, 24.5,ymax_);
      	TLine *line5V6=new TLine(32,0, 30.5,ymax_);
      	TLine *line6V6=new TLine(38,0, 36.5,ymax_);
      	TLine *line7V6=new TLine(45,0, 43.5,ymax_);
	...
	pad3->cd(); pad3->SetGridx(0);// pad2->SetGridy(0);
	line1V6->Draw();      line2V6->Draw();  line3V6->Draw();
	line4V6->Draw();      line5V6->Draw(); line6V6->Draw();
	line7V6->Draw(); line8V6->Draw();
	line9V6->Draw(); line10V6->Draw();
