...
void plotdatavsMC(TString varName,  TString year,  TString ntuples , bool ExpvsPred = false, bool SRvsCR=true)
{
  ...
  if(ntuples=="v6_elec")   path="rootoutput/v6/withphicut_zmass60to120_elec/" , elec=true, muon=false;
  if(ntuples=="v7_elec")    path="rootoutput/v7_btagSF/withphicut_zmass60to120_elec/" , elec=true, muon=false;  
  if(ntuples=="v8_elec")   path="rootoutput/v8_btagSFuperr/withphicut_zmass60to120_elec/" , elec=true, muon=false;

  if(ntuples=="v6_muon")   path="rootoutput/v6/withphicut_zmass60to120_muon/" , muon=true, elec=false;
  if(ntuples=="v7_muon")    path="rootoutput/v7_btagSF/withphicut_zmass60to120_muon/" , muon=true, elec=false;
  if(ntuples=="v8_muon")   path="rootoutput/v8_btagSFuperr/withphicut_zmass60to120_muon/" , muon=true, elec=false;
  
  if(ntuples=="v6_data")   path="rootoutput/v6/withphicut_zmass60to120_data/" , muon=false, elec=false;
  if(ntuples=="v7_data")    path="rootoutput/v7_btagSF/withphicut_zmass60to120_data/" , muon=false, elec=false;
  if(ntuples=="v8_data")   path="rootoutput/v8_btagSFuperr/withphicut_zmass60to120_data/" , muon=false, elec=false;
  ...
  if(year == "full_Run2")
    {
      ...
      filename2= path+"ZJZGToLL_CR_v18.root";
  ...
  if(varName=="nJets" || varName=="nvtx" || varName=="nJets_EW" ||varName=="nJets_SP"||varName=="Qmulti" )
   {
    ...
    rebin=1;
    ymin=0 , ymax=4.99, ymin_=0.05 , ymax_=100;
    // ymin_=0.0005 , ymax_=100000;
    pred_sr->Rebin(rebin);
    sr->Rebin(rebin);
    h_0->Rebin(rebin);
  ...
  pred_sr->SetTitle(title);
 ...
 else if(varName=="MET" || varName=="JetPt" || varName=="METvBin2" || varName=="JetPt1" || varName=="JetPt2" || varName=="MET_EW" ||varName=="hadAk8jetPt" )
   {
     title="MET";
      ...
      if(varName=="MET" || varName=="MET_EW")    xmin_=200,xmax_=1200,xmin=200,xmax=1200,rebin=5;
      ...
      //  rebin=1;
    ymin=0 , ymax=2.99, ymin_=0.05 , ymax_=20;
    pred_sr->Rebin(rebin);
    sr->Rebin(rebin);
    h_0->Rebin(rebin);
    h_1->Rebin(rebin);
 ...
 /*  cout<<"==============="<<endl; */
  ...
  double integeral=h_4->Integral();
  //  double integeral1=h_1->Integral() + h_2->Integral() + h_3->Integral();
  double integeral2=pred_sr->Integral();
  ...
  sprintf(chi2,"MC DY = %f",integeral);
  sprintf(chi3,"MC = %f, Data = %f",integeral2,integeral1);

  // TGraphAsymmErrors *h_grAE;
  ...
  cout<<"Chi ===> "<<chi<<endl;
  cout<<"Chi1 ===> "<<chi1<<endl;
  cout<<"Total events in Z/DY+gamma : "<<chi2<<endl;
  ...
  for(int i=0;i<=5;i++) 
   { cout<<" Data = "<<sr->GetBinContent(i)<<" : MC = "<<pred_sr->GetBinContent(i)<<" : "<<h3->GetBinContent(i)<<endl;} 
  /* cout<<"==============="<<endl; */
