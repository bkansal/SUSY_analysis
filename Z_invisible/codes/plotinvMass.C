...
void plotinvMass(){
  ...
  f[1] = new TFile(filename1);
  f[2] = new TFile(filename2);
  ...
  for(int i=0;i<nfiles;i++){
    ...
    //  h_MET->GetYaxis()->SetRangeUser(0,25);
    h_MET->Rebin(rebin);
    ...
    if(i==nBG-1) {
      ...
      hs_var->Draw("HISTE");
      hs_var->SetMinimum(0);
      hs_var->SetMaximum(25);
      decorate(hs_var,i,f[i]->GetName());
      hs_var->GetXaxis()->SetRangeUser(20,150);
      hs_var->GetYaxis()->SetRangeUser(0,100);
    }
    if(i>=nBG){
      c_cA->cd();
...
void drawlegend(TH1D *hist,int i,const char* fname){
  ...
  TString lName=fname;
  cout<<lName<<endl;
  if(lName.Contains("ZJZG")){lName="Z(#nu#bar{#nu})+#gamma";}
  ...
  else if(lName.Contains("Run")){lName="Data";}
