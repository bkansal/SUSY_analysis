void plotsys(TString sys="JECup",TString year="full_Run2",int xmin=1,int xmax=5)
{
  // gStyle->SetOptStat(0);
    gStyle->SetPalette(1);
 ...
 if(sys=="EWKcorr")
 ...
 else if(sys=="lepSF"|| sys=="JECup" || sys=="JERup")
   {
     ...
     double ymin=-0.12 , ymax=0.12, ymin_=-200 , ymax_=1600;
     ...
     TH1D *JERC = new TH1D("JERC","JERC systematics",52,0,52) ;
     if((sys=="JECup" || sys=="JERup" || sys=="lepSF") && varName=="AllSBins_v6_CD_EW_7bin_elec1_closure")
       {
	 fout =new TFile(rootout,"recreate");
	 fout->cd();
	 for(int i=1; i<=46; i++)
	   if(i<18) {JERC->SetBinContent(i,abs(sr1->GetBinContent(1))); JERC->SetBinError(i,abs(pull2->GetBinContent(1)));}
	   else if(i>=18 && i<33 ) {JERC->SetBinContent(i,abs(sr1->GetBinContent(2))); JERC->SetBinError(i,abs(pull2->GetBinContent(2)));} 
	   else if(i>=33 && i<39) {JERC->SetBinContent(i,abs(sr1->GetBinContent(3))); JERC->SetBinError(i,abs(pull2->GetBinContent(3)));}
	   else {JERC->SetBinContent(i,abs(sr1->GetBinContent(4))); JERC->SetBinError(i,abs(pull2->GetBinContent(4)));}
	 JERC->Write();
