//For Predicted events in SR region using Data , N pred.data(SR)=purity factor * TF * normfacor * N obs.data(CR)
//put scale = false; and run following :
//root -l 'plotclosure.C("AllSBins_v6_CD_EW_50bin","full_Run2","v17",false,true)'
//
// For transfer factor,
//put scale = false; and run following :   
//  root -l 'plotclosure.C("AllSBins_v6_CD_EW_7bin","full_Run2","v18",false,true)'   
//
//For norm factor,
//put scale = true; and run following :        
//root -l 'plotclosure.C("AllSBins_v6_CD_EW_50bin","full_Run2","v18",false,true)'                     
//
//For Scale factor,
//put scale = false; and run following :
//  root -l 'plotclosure.C("AllSBins_v6_CD_EW_7bin","full_Run2","v17",true,false)'    


void plotclosure(TString varName,  TString year,  TString ntuples , TString dir2, bool ExpvsPred = true, bool SRvsCR=false)
{
  // gStyle->SetOptStat(0);
  ...
  if(ExpvsPred)
    {
      png = path+"/png/closure/"+varName+"_"+year+".pdf";
      pdf= path+"/Pdf/closure/"+varName+"_"+year+".png";
    }
  else if(SRvsCR)
    {
      png = path+"/png/Zin_"+varName+"_"+year+"_SF.pdf";
      pdf= path+"/pdf/Zin_"+varName+"_"+year+"_SF.png";
    }
  ...
  xmin_=1,xmax_=37,xmin=1,xmax = 37,bin=36;
   ...
   bin=4;
    ...
    if(varName=="BestPhotonPt") {rebin=1;title="p^{#gamma}_{T} (GeV)"; xmin_=100,xmax_=1400,xmin=100,xmax=1400; }
    else rebin=5;
  ...
  if(ntuples!="v18" && ExpvsPred)
   ...
   if(ntuples!="v18" && SRvsCR)
    {
      cout<<"Integral for Pred. Data (SR) : "<<pred_sr->Integral()<<endl;
      cout<<"Integral for MC(CR): "<<sr->Integral()<<endl;
    }
  ...
  if(varName=="AllSBins_v6_CD"){
    ...
    TLatex Tl;
    Tl.SetTextSize(0.06);
    Tl.DrawLatex(1.5,2*yset_,"N_{BTags} = 0");
    Tl.DrawLatex(2.5,2*yset_,"N_{BTags} > 0");
  ...
  if(SRvsCR)
	...
	if(ntuples=="v18")
	  ...
	  {
	    legend->AddEntry(sr,"#beta x Z -> l^{+}l^{-} + #gamma (data) ","lp");
	    legend->AddEntry(pred_sr,"Z ->l^{+}l^{-} + #gamma (MC)","lp");
