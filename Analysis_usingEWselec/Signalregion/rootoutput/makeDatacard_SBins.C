...
void makeDatacard_SBins(double mGl,double mNLSP,TString sigFile){
  ...
  f[1] = new TFile("WGJetsToLNuG_v17.root");
  f[2] = new TFile("WJetsToLNu_v17.root");
  f[3] = new TFile("TTJetsHT_v17.root");
  f[4] = new TFile("TTGJets_v17.root");
  f[5] = new TFile("ZGZJ_NuNuG.root");
  //f[5] = new TFile("ZGJetsToNuNuG_v17.root");
  //f[6] = new TFile("ZJetsToNuNu_v17.root");
  f[6] = new TFile("GJetsQCD_new_v17.root");
  //  f[7] = new TFile("QCD_v17.root");

  //  char histname[100]="MET_R1";
  char histname[100]="AllSBins_v7_CD";
  TH1D *hist1[nFiles];
  ...
  for(int i=1;i<=imax;i++){
    ...
    string name2="dataCards/"+getfname(f[0]->GetName())+"_"+histname+"/"+getfname(f[0]->GetName())+"_"+histname+"_bin"+to_string(i)+".txt";
    cout<<name2<<endl;
