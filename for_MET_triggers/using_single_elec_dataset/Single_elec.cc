...
void Single_elec::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for(int i=0;i<TriggerNames->size();i++){
      ...
      // if( trgName.Contains("HLT_Ele32_eta2p1_WPTight_Gsf_v") && (*TriggerPass)[i]==1 )         { h_selectBaselineYields_->Fill(2); passTrg = true;}
      ...
      if( trgName.Contains("HLT_PFHT300_PFMET100") && (*TriggerPass)[i]==1 )      { h_selectBaselineYields_->Fill(7); passHTxPhoTrg = true;}
      else if( trgName.Contains("HLT_PFHT300_PFMET110") && (*TriggerPass)[i]==1 )      { h_selectBaselineYields_->Fill(8); passHTxPhoTrg = true;}
