#ifndef Single_muon_H
#define Single_muon_H
...
class Single_muon : public NtupleVariables{

 public:
  Single_muon(const TString &inputFileList="foo.txt", const char *outFileName="histo.root",const char *dataset="data");
  ~Single_muon();
...
#ifdef Single_muon_cxx

void Single_muon::BookHistogram(const char *outFileName) {
  ...
  h_METvBin=new TH1D("METvBin","MET in variable bins for events passing denominator trigger",PhoPtLowEdge.size()-1,&(PhoPtLowEdge[0]));
  ...
  h_METvBin_Req=new TH1D("METvBin_Req","MET in variable bins for events passing trigger_Req",PhoPtLowEdge.size()-1,&(PhoPtLowEdge[0]));
...
Single_muon::Single_muon(const TString &inputFileList, const char *outFileName, const char* dataset) {
...
Bool_t Single_muon::FillChain(TChain *chain, const TString &inputFileList) {
...
Long64_t Single_muon::LoadTree(Long64_t entry) {
