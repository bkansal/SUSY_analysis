...
void printRK_0pho(TString varname = "AllSBins_v6_CD_EW_50bin", int xmin=1, int xmax=46, int rebin=1)
{
  ...
  //  bool Kvalid=false, ExpvsPred=true;
  //   bool Kvalid=false, ExpvsPred=true, SRvsCR=false;
 ...
 TH1D *JERC;
  ...
  // TH1D *LL_low=(TH1D*)LL_lowdphi->FindObjectAny("AllSBins_v6_CD_EW_50bin_AB"); 
  //  TH1D *LL_high=(TH1D*)LL_highdphi->FindObjectAny("AllSBins_v6_CD_EW_50bin_CD"); 
  ...
  */
  ...
  if(ExpvsPred){
    ...
    TString varname1=varname+"_B";
    TString varname2=varname+"_D";
  ...
  if(Kvalid)
    ...
    TH1D *K_data = new TH1D("K_data","K factors in data",9,0,9);
    TH1D *K_mc = new TH1D("K_mc","K factors in mc",9,0,9);
    for(int i=0;i<3;i++)
      {
	...
	cout<<i<<" : "<<R_highdphi_nb1_data->GetBinContent(2,i+1)<<" : "<<R_highdphi_nb1_data->GetBinContent(1,i+1)<<endl;
    ...
    int xmin=0, xmax=8;
    double ymin=0 , ymax=1.99, ymin_=0 , ymax_=3;
    TCanvas *c1;
    gStyle->SetOptStat(0);
    c1 = new TCanvas("stackhist","stackhist",800,600);
    TPad *pad1 = new TPad("pad1","pad1",0,0,1,1);
    ...
    K_mc->GetYaxis()->SetLabelSize(0.07);
    K_data->GetYaxis()->SetTitle("K factor");
    K_data->GetYaxis()->SetTitleSize(0.04);
    K_data->GetYaxis()->SetTitleOffset(0.92);
  ...
  cout<<"yset = "<<yset<<endl;
  TArrow *arrow0 = new TArrow( 0, yset, 1, yset,0.01,"<|>");
  ...
  yset=(ymax_+ymin_)/2.1;
  cout<<"yset = "<<yset<<endl;
  TLatex Tl,T2;
  Tl.SetTextSize(0.035);
  T2.SetTextSize(0.035);
  Tl.DrawLatex(0.25,yset,"N^{ 0}_{ 2}");
  Tl.DrawLatex(1.25,yset,"N^{ 0}_{ 5-6}");
  Tl.DrawLatex(2.25,yset,"N^{ 0}_{ #geq7}");
  Tl.DrawLatex(3.25,yset,"N^{ #geq1}_{ 2-4}");
  Tl.DrawLatex(4.25,yset,"N^{ #geq1}_{ 5-6}");
  Tl.DrawLatex(5.25,yset,"N^{ #geq1}_{ #geq7}");
  Tl.DrawLatex(6.25,yset,"W tag");
  Tl.DrawLatex(7.25,yset,"H tag");
  T2.DrawLatex(2.5,yset*1.5,"SP region");
  T2.DrawLatex(6.5,yset*1.5,"EW region");
    
  TLegend *legend1 = new TLegend(0.55,0.78,0.9,0.9);
  legend1->AddEntry(K_data,"K(N_{jets},N_{b-tags},EW) data","lp");
  legend1->AddEntry(K_mc,"K(N_{jets},N_{b-tags},EW) MC","lp");
  ...
  */
  ...
  h3->GetXaxis()->SetTitleSize(0.18);
  h3->GetXaxis()->SetLabelSize(0.12);
  h3->GetYaxis()->SetTitle("K_{data}/K_{MC}");
  h3->GetYaxis()->SetTitleSize(0.15);
  h3->GetYaxis()->SetTitleOffset(0.28);
  h3->GetYaxis()->SetLabelSize(0.1);
  h3->GetYaxis()->SetNdivisions(5);
  ...
  for(int i=xmin;i<=xmax;i++)
    h5->SetBinContent(i,1-abs(h3->GetBinContent(i)));
  h5->Print("all");
