...
void printRK(bool R_data=true, bool syst=false, bool RK=false)
{
  //  bool R_data=false;
  //bool RK=true;
  TString path1="../lostlepton/Lostlepton_lowdphi.root";
  TString path2="../fakerate/Fakerate_lowdphi.root";
  TString path3="../Z_invisible/Zinvi_lowdphi.root";
  TString path1_="../lostlepton/Lostlepton_highdphi.root";
  TString path2_="../fakerate/Fakerate_highdphi.root";
  TString path3_="../Z_invisible/Zinvi_highdphi.root";
  TString pdf,png;
  ...
  TFile *Multi_data = TFile::Open("rootoutput/btagSF/Run2_METdata_CR_v18.root","READ");
  TFile *data = TFile::Open("rootoutput/v2_btagSF/ST300_Rfactor_applied_noRK/Run2_METdata_CR_v18.root","READ");
  ...
  TFile *data_pred = TFile::Open("rootoutput/final/RK_MCclosure/Run2_METdata_CR_v18.root","READ");
  ...
  /*
  cout<<"Predictions in highdphi : "<<endl;
  //  for(int i=1;i<46;i++)
  ewk->Print("all");
  cout<<"Predictions in lowdphi : "<<endl;
  ewk_low->Print("all");
  */
  TFile *K_valid = TFile::Open("factors/K_validation.root","READ");
  TH1D *syserror=(TH1D*)K_valid->FindObjectAny("k_valid");
  //  TFile *Multi = TFile::Open("rootoutput/v2_btagSF/ST300_Rfactor_applied_noRK/Run2_METdata_CR_v18.root","READ");
  //TFile *Multi_mc = TFile::Open("rootoutput/v2_btagSF/ST300_Rfactor_applied_noRK/GJetsQCD_new_v18.root","READ");
  TFile *Multi = TFile::Open("rootoutput/btagSF/Run2_METdata_CR_v18.root","READ");
   TFile *Multi_mc = TFile::Open("rootoutput/btagSF/GJetsQCD_new_v18.root","READ");
  //  TFile *Multi_mc = TFile::Open("rootoutput/final/noRK/GJetsQCD_new_v18.root","READ");
  ...
  TFile *f1;
  if(!syst) f1=new TFile("RK_lowMET_data.root","recreate");
  else f1=new TFile("RK_lowMET_data_sys.root","recreate");
  ...
  //  TFile *data = TFile::Open("rootoutput/btagSF/Run2_METdata_CR_v18.root","READ");
  //TH1D *cr=(TH1D*)data->FindObjectAny("AllSBins_v6_CD_EW_50bin_AB");
  ...
  for(int i=1;i<=46;i++)
	...
	else AllSBins_v6_CD_EW_50bin_elec1_closure->SetBinError(i,AllSBins_v6_CD_EW_50bin_elec1_closure->GetBinContent(i)*pow(pow(AllSBins_v6_CD_EW_50bin_elec1->GetBinError(i)/AllSBins_v6_CD_EW_50bin_elec1->GetBinContent(i),2) + pow(fr->GetBinError(i)/fr->GetBinContent(i),2) + pow(syserror->GetBinContent(i),2),0.5));
  ...
  if(R_data)
      ...
      for(int i=0;i<8;i++)
	{
      ...
      C_data->GetYaxis()->SetRangeUser(0.1,250000);
      C_data->SetLineColor(kBlue);
      A_data->SetLineColor(kRed);
      C_data->SetTitle(0);
      C_data->GetYaxis()->SetTitle("Entries");
      C_data->Draw();
      A_data->Draw("sames");
      ...
      TArrow *arrow11 = new TArrow( 1,1.5, 7,1.5,0.01,"<|>");
      TArrow *arrow12 = new TArrow( 7,1.5, 9,1.5,0.01,"<|>");
      ...
      Tl.DrawLatex(7.25,yset,"W tag");
      Tl.DrawLatex(8.25,yset,"H tag");
      T2.DrawLatex(2.5,2,"SP region");
      T2.DrawLatex(7.5,2,"EW region");
      TLegend *legend1 = new TLegend(0.55,0.78,0.9,0.9);
      legend1->AddEntry(C_data,"N_{C}(low #Delta #Phi & low MET)","lp");
      legend1->AddEntry(A_data,"N_{A}(high #Delta #Phi & low MET) ","lp");
      ...
      RlowMET_data->GetXaxis()->SetTitle("Bin no.");
      RlowMET_data->GetXaxis()->SetTitleOffset(0.8);
      RlowMET_data->GetXaxis()->SetTitleSize(0.18);
      RlowMET_data->GetXaxis()->SetLabelSize(0.12);
      RlowMET_data->GetYaxis()->SetTitle("R=N_{C}/N_{A}");
      RlowMET_data->GetYaxis()->SetTitleSize(0.15);
    ...
    if(RK)
    {
      ...
      pdf="../plots/pdf/multi/Multi_K.pdf";
      png="../plots/png/multi/Multi_K.png";
        TLatex textOnTop,intLumiE;
      TCanvas *c1= new TCanvas("stackhist","stackhist",800,600); 
      c1->cd();
      gStyle->SetOptStat(0);
      ...
      D_MC->GetYaxis()->SetRangeUser(0,2);
      D_MC->SetLineColor(kBlue);
      C_MC->SetLineColor(kRed);
      D_MC->SetTitle(0);
      D_MC->GetYaxis()->SetTitle("R(nJ,nbj,EW)");
      D_MC->Draw("ep");
      C_MC->Draw("ep sames");
      ...
      TLegend *legend1 = new TLegend(0.65,0.78,0.9,0.9);
      legend1->AddEntry(D_MC,"R(high MET)","lp");
      legend1->AddEntry(C_MC,"R(low MET) ","lp");
      legend1->SetTextSize(0.035);
      legend1->Draw();
      textOnTop.SetTextSize(0.04);
      intLumiE.SetTextSize(0.04);
      ...
      pad2->SetGridy(0);
      RlowMET_data->SetTitle(0);
      RlowMET_data->SetMaximum(0.65);
      RlowMET_data->SetMinimum(-0.5);
      RlowMET_data->GetXaxis()->SetTitle("Bin no.");
      RlowMET_data->GetXaxis()->SetTitleOffset(0.8);
      RlowMET_data->GetXaxis()->SetTitleSize(0.18);
      RlowMET_data->GetXaxis()->SetLabelSize(0.12);
      RlowMET_data->GetYaxis()->SetTitle("K=#frac{R(high MET)}{R(low MET)}");
      RlowMET_data->GetYaxis()->SetTitleSize(0.1);
      RlowMET_data->GetYaxis()->SetTitleOffset(0.32);
      RlowMET_data->GetYaxis()->SetLabelSize(0.1);
      RlowMET_data->GetYaxis()->SetNdivisions(5);
      RlowMET_data->Draw("E0");
      c1->SaveAs(pdf);
