//void plotclosure_LL(TString varName,  TString year,  TString ntuples , bool ExpvsPred = true, bool SRvsCR=false)
//AllSBins_v6_CD_EW_50bin
//void plotclosure_LL(TString varName="AllSBins_v7_CD_SP",  TString year="full_Run2",  TString ntuples="v18" , bool ExpvsPred = false, bool SRvsCR=true)
void plotclosure_LL(TString varName="AllSBins_v6_CD_EW_50bin",  TString year="full_Run2",  TString ntuples="v18" , bool ExpvsPred = true, bool SRvsCR=false)
{
  ...
  if(ntuples=="v18")
    {
      ...
      path="final_rootfiles/LL/TF_v7_usingfullRun2_BTagSF_SF_newBin2/";
      //path="rootoutput/newselec_LL_noISRjet_METfilters_EW_hadjetID_newPU_v18/TF_v5_usingfullRun2_BTagDeepCSV_newBin2/";
      //      path="final_rootfiles/LL/TF_v6_usingfullRun2_BTagDeepCSV_SFuperror_newBin2/";
      path="rootoutput/new/tmp/LE_muo/";
      path1="rootoutput/new/tmp/LE_muo/";
      
      path="rootoutput/new/nominal/LL/";
      path1="rootoutput/new/nominal/LL/";      
      //      path="./";
    }
  else
    {
  ...
  if(ExpvsPred)
    {
      ...
      png="png/LL/LL_"+varName+"_"+year+"_closure.png";
      pdf="../plots/pdf/LL/LL_"+varName+"_"+year+"_closure.pdf";
 ...
 c1 = new TCanvas("stackhist","stackhist",1000,600);
	...
	if(ExpvsPred)   pred_sr     = (TH1D*)f2->Get("AllSBins_v6_CD_EW_50bin_elec0");
	if(SRvsCR)   pred_sr     = (TH1D*)f1->Get("AllSBins_v6_CD_EW_50bin_elec1");
      }
    ...
    ymin=0 , ymax=1.99, ymin_=0.005 , ymax_=10000000, yset_=5000;
    title="closure test using optimized search bins";
  pred_sr->SetTitle(0);
  }
  ...
  pred_sr->SetTitle(title);
    ...
    /*    if(data_pred)
      {
        sr          = (TH1D*)f1->Get("nJets_elec1_closure");
        if(ExpvsPred)   pred_sr     = (TH1D*)f2->Get("nJets");
        if(SRvsCR)   pred_sr     = (TH1D*)f2->Get("AllSBins_v6_CD_EW_50bin_elec1");
      }
    */
     if(varName=="ST")
      {
    ...
    pred_sr->Rebin(rebin);
    sr->Rebin(rebin);
    ymin=0 , ymax=1.99, ymin_=0.5 , ymax_=1000;
    pred_sr->GetXaxis()->SetRangeUser(xmin,xmax);
    sr->GetXaxis()->SetRangeUser(xmin,xmax);
    title="p^{miss}_{T}";
  pred_sr->SetTitle(title);
  ...
  //  if(!data_pred)
	...
	//cout<<setprecision(4)<<pred_sr->GetBinContent(i)<<" +- "<<pred_sr->GetBinError(i)<<endl;
       ...
       { cout<<sr->GetBinContent(i)<<endl;//" ====== "<<sr->GetBinContent(i)<<endl;} */
       } 
  ...
  */
  ...
  for(int i=bin0;i<=bin1;i++)
    { cout<<sr->GetBinContent(i)<<endl;}
  cout<<"The Pred. SR content in bin"<<endl;
  for(int i=bin0;i<=bin1;i++)
