void printall(bool lowdphi=false, TString year="full_Run2",int nbin=52, int xmin=0, int xmax=52)
{
  ...
  int n=10;
  bool print=true;
  TFile *f_syserr= new TFile("LL_sys.root");
  ...
  if(sys)
    {
      ...
      */
      filename5  = "LL_JECup_v2.root";
      filename6  = "LL_JERup_v2.root";
  ...
  for(int i=1;i<n;i++)
    {
      ...
      if(!lowdphi) h_preddata->SetBinError(i+1,pred_sr->GetBinContent(i+1)*pow((pow(cr->GetBinError(i+1)/cr->GetBinContent(i+1),2)+pow(frerr_mjb[i]/fr_mjb[i],2)+ pow(syserr->GetBinError(i)/pred_sr->GetBinContent(i+1),2)),0.5));
      //           h_preddata->SetBinError(i+1,pow((pow(cr->GetBinContent(i+1)*frerr_mjb[i],2) + pow(cr->GetBinError(i+1)*fr_mjb[i],2)),0.5));
...
inContent(i+1));
	  */
	  ...
	  h_jecsys->SetBinContent(i+1,abs(pred_sr_jec->GetBinContent(i+1)));
          h_jersys->SetBinContent(i+1,abs(pred_sr_jer->GetBinContent(i+1)));
