...
void plotdatavsMC_LL(TString varName, TString year, TString ntuple, bool DatavsMC=false)
{
  TFile *f1, *f2,*f3,*f4;
  TFile *f_0,*f_1,*f_2,*f_3,*f_4;

  TLatex textOnTop,intLumiE;
  bool isPaper=false, AN=false, progress=false;
  ...
  if(DatavsMC)
    {
      png="../plots/png/LL/LL_"+varName+"_"+year+"_datavsMC.png";
      pdf="../plots/pdf/LL/LL_"+varName+"_"+year+"_datavsMC.pdf";
    }
  if(!DatavsMC)
  ...
  //   for(int i=0;i<=53;i++) 
  // { cout<<i<<" , CR : "<<sr->GetBinContent(i)<<" , Pred : SR : "<<pred_sr->GetBinContent(i)<<endl;} 
  ...
  else if(AN) textOnTop.DrawLatexNDC(0.12,0.91,"CMS #it{#bf{Preliminary}}");
