void printyear(bool lowdphi=false, TString year="2016",int nbin=52, int xmin=0, int xmax=52)
{
  ...
  if(!lowdphi)    TF = TFile::Open("factors/lostlepton_highdphi.root","READ");
  if(lowdphi)    TF = TFile::Open("factors/lostlepton_lowdphi.root","READ");
  ...
  if(sys)
    {
      ...
      */
      filename5  = "LL_JECup_"+year+"_v2.root";
      filename6  = "LL_JERup_"+year+"_v2.root";
  ...
  TFile *fout;
  TString fname;
  if(!lowdphi) fname="Lostlepton_highdphi_"+year+".root";
  if(lowdphi)  fname="Lostlepton_lowdphi_"+year+".root";
