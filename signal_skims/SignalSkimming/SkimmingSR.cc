...
void SkimmingSR::EventLoop(const char *data,const char *inputFileList,const double mGl,const double mNLSP) {
 ...
 //  // TFile *f1 = new TFile("T5qqqqHg_Summer16v3_MassScan.root");
 // TFile *f1 = new TFile("T5ttttZG_Summer16v3Fast_MassScan.root");
 // // TFile *f1 = new TFile("T5bbbbZG_Fall17Fast.SMS-T5bbbbZg_TuneCP2_13TeV-madgraphMLM-pythia8_job0.root");
 // //TFile *f1 = new TFile("T5bbbbZG_Autumn18Fast.SMS-T5bbbbZg_TuneCP2_13TeV-madgraphMLM-pythia8_job0.root");
 // TFile *f2 = new TFile("mGl_Xsecpb_absUnc.root");
 // TH1D *h1_xsechist = (TH1D*)f2->FindObjectAny("mGlXsec");
 ...
 //  TFile *f1 = new TFile("TChiNG_Summer16v3Fast.SMS-TChiNG_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_job0.root");
 // // TFile *f1 = new TFile("TChiNG_Fall17_MassScan.root");
 //  TFile *f2 = new TFile("EWeakino_N2N1Xsec.root");
 //  TH1D *h1_xsechist = (TH1D*)f2->FindObjectAny("mEWeakino_N2N1Xsec");//N2C1 for TChiWG mEWeakino_N2C1Xsec
 // // TH1D *h1_xsechist = (TH1D*)f2->FindObjectAny("mEWeakino_SumN2C1_C1C1Xsec");//N2C1 for TChiNG
 //  TH2D *h2_mass = (TH2D*)f1->FindObjectAny("MGlMNLSP");
 //  if(!h1_xsechist){ cout<<"AHHHHH: could not find xsec hist"<<endl; return;}
 //  if(!h2_mass){ cout<<"AHHHHH: could not find no. of events hist"<<endl; return;}
 //  xsec1 = h1_xsechist->GetBinContent(h1_xsechist->FindBin(nlspMass));
  //----------------------- for Stop models ----------------------------------
  TFile *f1 = new TFile("T6ttZG_Summer16v3_MassScan.root");
  TFile *f2 = new TFile("mStop_Xsecpb_absUnc.root");                                                                             
  TH1D *h1_xsechist = (TH1D*)f2->FindObjectAny("mStopXsec");
  TH2D *h2_mass = (TH2D*)f1->FindObjectAny("MGlMNLSP");
  if(!h1_xsechist){ cout<<"AHHHHH: could not find xsec hist"<<endl; return;}
  if(!h2_mass){ cout<<"AHHHHH: could not find no. of events hist"<<endl; return;}
  xsec1 = h1_xsechist->GetBinContent(h1_xsechist->FindBin(momMass));
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(abs(SusyMotherMass-momMass) > 0.001 || abs(SusyLSPMass-nlspMass) > 0.001)
      {
	continue;
      }
    cout<<"SusyMotherMass:"<<SusyMotherMass<<" SusyLSPMass:"<<SusyLSPMass<<endl;
